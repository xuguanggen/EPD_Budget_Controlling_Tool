...
<html>
	<head>
		...
		function newSubmit()
		{
			...
			{
			   document.form1.action =contextPath+"/AllChartAction!create";
		...
		function makeothersChecked() {
			...
			if (x[0].checked == true) {
				...
				if(chestr!=""){
					document.form1.action = "/EPD_Budget_Controlling_Tool/AllChartAction!create";
					document.form1.submit();
				}
			} else {
				x[1].checked = false;
									...
									<div id="display" style="width: 100%;">
										...
										<div id="comtable">
													...
													<tr style="height: 5px">
														<td align="center">
															...
															<%
																...
																int selectYear = 0;
																if (session.getAttribute(ip + "chartYear") != null) {
																	selectYear = Integer.parseInt(session.getAttribute(
																			ip + "chartYear").toString().trim());
																}
																out
														...
														<td width="10%">
															<select name="tpmname" id="tpmname"
																...
																<%
																	String empnameString = session.getAttribute(ip + "EmpName")
																			.toString();
																	String tpmname = "";
																	if (session.getAttribute("tpmname" + empnameString) != null) {
																	...
																	{
																	  out.println("<option value='70818773' selected>Ganbour Munir</option>");
																	}else
																	{
																	   out.println("<option value='70818773'>Ganbour Munir</option>");
																	}
																%>
															</select>
														</td>
															...
															<div id="pro"
																...
																<%
																	if (request.getAttribute("projectList") != null) {
																		ArrayList<String> al = (ArrayList<String>) request.getAttribute("projectList");
											...
											<%
											    ...
											    String aeName=(String)request.getAttribute("aeFileName");
											    String rbeiName=(String)request.getAttribute("rbeiName");
											    if(request.getAttribute("aeFileName")!=null)
											    {
											       aeName=request.getAttribute("aeFileName").toString().trim();
											    }
											    if(request.getAttribute("rbeiName")!=null)
											    {
											       rbeiName=request.getAttribute("rbeiName").toString().trim();
											    }
											...
											String tgraphUrl=request.getContextPath()
																+ "/chart/DisplayChart?filename=" + totalName;										    
											%>
											<img src="<%=tgraphUrl%>" usemap="#'<%=totalName%>'">
											<%
											}
											if(aeName!=null)
											{
											  String aePie=request.getAttribute("aepie").toString().trim();
											  String graphURL = request.getContextPath()
																+ "/chart/DisplayChart?filename=" +aeName;
											  String pieAeUrl=request.getContextPath()
																+ "/chart/DisplayChart?filename=" +aePie;
											%>
											<img src="<%=graphURL%>" usemap="#'<%=aeName%>'">
											<img src="<%=pieAeUrl%>" usemap="#'<%=aePie%>'">
											<%
											}
											if(rbeiName!=null)
											{
											  String rbeiPie=request.getAttribute("rbeiPie").toString().trim();
											  String graphURL = request.getContextPath()
																+ "/chart/DisplayChart?filename=" +rbeiName;
											  String pieRbeiUrl=request.getContextPath()
																+ "/chart/DisplayChart?filename=" +rbeiPie;
											%>
											<img src="<%=graphURL%>" usemap="#'<%=rbeiName%>'">
											<img src="<%=pieRbeiUrl%>" usemap="#'<%=rbeiPie%>'">
											<%
											}
											%>											
										</div>
